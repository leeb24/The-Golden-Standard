...
{
	if(tcb_storage[thread].state != 0) //if the thread state is not exited 
	{
	
		tcb_storage[threadtracker].state = 3; //block the thread
	
		tcb_storage[thread].jointhread = threadtracker; // remember what thread to join after target thread terminates

	
		signal_handler(); //schedule 

		if(value_ptr != NULL)
			*value_ptr = tcb_storage[thread].exitreturn; // set value pointer to the return value 
	}
	//schedule();
...
{	
	...
	pshared = 0; // always default to 0

	sem1.value =value; // initial semaphor value 
 
	sem->__align = &sem1; //Keep reference to the user made semaphor 

	sem1.semainitial = 1; // INITIALIZED 

	sem1.semacount = 0; // For the queue 
	sem1.semafront  = 0;
	sem1.semaback = -1;
...
int sem_wait(sem_t *sem)
{
	if(sem1.value > 0 )
	{
	  sem1.value--; // Decrement semaphor value 
	}
	else
	{
	  insert(threadtracker); //put the thread ID into the queue 
	  tcb_storage[threadtracker].state = 3; // BLOCK  the thread form accessing the resource 
	}
...
{
	sem1.value++; // Increase semaphor value 

	int popthread = removeData(); //pop the queue 
...
{
	sem1.semainitial = 0; //Destroy the semaphor 
}
